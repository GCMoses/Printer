...
namespace Printer.Web.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string? returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (Input.Role == Statics.Role_User_Comp)
                {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (Input.Role == null)
                    {
                        await _userManager.AddToRoleAsync(user, Statics.Role_User_Indi);
                    }
                    else
                    {
                    ...
                    string companyName = "Digi-Pix";
                    string currentDateTime = DateTime.Now.ToString("dddd, MMMM dd yyyy hh:mm tt");
                    string messageBody = $@"
    <html>
        <body>
            ...
            <a href='{HtmlEncoder.Default.Encode(callbackUrl)}'>
                <button style='background-color: #008CBA; border: none; color: white; padding: 15px 32px; text-align: center; text-decoration: none; display: inline-block; font-size: 16px; margin: 4px 2px; cursor: pointer;'>Confirm Account</button>
            </a>
            <p>This email was sent on {currentDateTime}.</p>
        </body>
    </html>
";
